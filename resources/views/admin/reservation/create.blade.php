@extends('admin.layouts.admin' , ['sectionName' => 'ایجاد رزرو'])
    ...
    <div class="card row">
        <div class="card-body px-5 py-4">
            <div class="d-flex justify-content-between">
                <div><h3 class="card-title mb-3">ایجاد رزرو</h3></div>
            ...
            <form action="{{route('admin.reservation.store')}}" method="post">
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="number" class="form-control" id="number" value="{{old('number')}}" placeholder="شماره همراه">
                        @error('number')
                        <div class="alert-danger">{{$message}}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="start_time" class="form-control" value="{{old('start_time')}}" id="start_time" placeholder="زمان شروع"
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="end_time" class="form-control" value="{{old('end_time')}}" id="end_time" placeholder="زمان پایان"
                               onkeypress="return false">
                        @error('end_time')
                        <div class="error text-danger">{{$message}}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select name="number_of_person" id="number_of_person" class="form-control">
                            @foreach(range(1,50) as $num)
                                <option value="{{$num}}" @selected(old('number_of_person') === $num)>{{$num}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <div class="form-check">
                            <label for="is_pay" class="form-check-label">
                                <input type="checkbox" name="is_pay" id="is_pay" class="form-check-input" @checked(old('is_pay') == 'on')>پرداخت شده
                            </label>
                        </div>
                        @error('is_pay')
                        <div class="error text-danger">{{$message}}</div>
