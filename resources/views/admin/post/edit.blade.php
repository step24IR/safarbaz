@extends('admin.layouts.admin' , ['sectionName' => 'ویرایش پست'])
    ...
    <div class="card row">
        <div class="card-body px-5 py-4">
            <div class="d-flex justify-content-between">
                <div><h3 class="card-title mb-3">ویرایش پست</h3></div>
            ...
            <form action="{{route('admin.post.update' , ['post' => $post->id])}}" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="form-group exclusive">
                        <div class="form-check">
                            <label for="published" class="form-check-label">
                                <input type="checkbox" name="published" id="published" class="form-check-input" @checked($post->published === 1)>انتشار
                            </label>
                        </div>
                        @error('published')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-control" id="title" value="{{$post->title}}" placeholder="موضوع">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="slug" class="form-control" id="slug" value="{{$post->slug}}" placeholder="اسلاگ">
                        @error('slug')
                        <div class="alert-danger">{{$message}}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        <label>متن:</label>
                        <div class=" text-dark">
                            <textarea name="text" id="editor" placeholder="توضیحات" rows="3">{{$post->text}}</textarea>
                        </div>
                        @error('text')
                        <div class="error text-danger">{{$message}}</div>
                        @enderror
